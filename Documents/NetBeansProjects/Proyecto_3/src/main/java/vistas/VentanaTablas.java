...
public class VentanaTablas extends javax.swing.JInternalFrame {
    ...
    public VentanaTablas(ControladorMascota mascotaCtrl, ControladorPersona controladorPersona) {
    ...
    public void ListarMascota() {
        ...
        List<MascotasDTO> lista = mascotaControlador.listarMascotas();
    ...
    public void ListarPropietario() {
        ...
        List<PersonaDTO> lista = controladorPersona.listar();
        for (PersonaDTO p : lista) {
            if(p instanceof PropietarioDTO prop){
    ...
    int fila = tablaPropietarios.getSelectedRow();
       ...
       if (confirm == JOptionPane.YES_OPTION) {
           boolean eliminado = controladorPersona.eliminarPersona(documento);
           if (eliminado) {
...
String documento = modeloPropietarios.getValueAt(fila, 1).toString(); // Asegúrate que la columna 1 sea el código
...
// Buscar persona en la lista por código
PersonaDTO persona = null;
List<PersonaDTO> listaPersonas = controladorPersona.listar();

for (PersonaDTO p : listaPersonas) {
    if (p.getCodigo().equals(documento)) {
        persona = p;
        break;
    }
}

// Validar si existe y es un propietario
if (persona == null) {
    JOptionPane.showMessageDialog(this, "No se encontró una persona con ese documento.", "Error", JOptionPane.ERROR_MESSAGE);
    return;
}

if (!(persona instanceof PropietarioDTO)) {
    JOptionPane.showMessageDialog(this, "Solo se pueden editar propietarios desde esta vista.", "Advertencia", JOptionPane.WARNING_MESSAGE);
    return;
}

// Crear objeto actualizado y enviar al controlador
PersonaDTO propietarioActualizado = new PersonaDTO(nuevoNombre, nuevoTelefono, documento);

boolean actualizado = controladorPersona.editarPersona(documento, propietarioActualizado);
...
if (actualizado) {
    ...
    // Actualizar
    MascotasDTO mascota = new MascotasDTO (nuevoNombre, nuevaEspecie, nuevaEdad,docPropActual, id);

    boolean actualizado = mascotaControlador.editarMascota(id,mascota);
    if (actualizado) {
